...
from utils.data_pred import get_contexts, get_data, get_lr, predict, predict_from_saved, preprocess
...
def rename_move(data, colab=False, save_models=False):
    if colab:
        folder = "Context-Aware-Stereotype-Detection-Conversational-Thread-Analysis"
    else:
        folder = ""
    ...
    # Move CSV files
    csv_source_dir = os.path.join("Context-Aware-Stereotype-Detection-Conversational-Thread-Analysis", "results", data)
    ...
    for seed, (data, soft_labels) in tqdm(SEEDS_DATA_SOFT, desc="data loop", position=0):
        ...
        for model_name, model_checkpoint in tqdm(
            ...
            for context in tqdm(data_contexts, desc="context loop", position=2, leave=False):
                ...
                if H_PARAMS["temperature_scaling"] and not soft_labels:
                    model_temp = ModelWithTemperature(trainer.model, GLOBALS)
                    model_temp.set_temperature(tokenizer, val_set)
                    temperature = float(model_temp.temperature.item())
                    del model_temp
                    torch.cuda.empty_cache()
                    pred_labels, _, softmax_confidence = predict_from_saved(
                        trainer.model,
                        tokenizer,
                        test_set,
                        GLOBALS["device"],
