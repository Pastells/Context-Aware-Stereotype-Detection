...
def join_wrapper(file: str, data: str) -> pd.DataFrame:
    if data == "detests":
        return join_detests(file)
    elif data == "stereohoax":
        return join_by_index(file, ST_TEST)
    else:
        raise ValueError("detests or stereohoax")


def join_detests(file: str) -> pd.DataFrame:
    """Join results file with rest of test columns for DETESTS"""
    df = pd.read_csv(file)
    columns = [c for c in df.columns[2:] if c in DET_TEST.columns]
    df = df.merge(
        DET_TEST.drop(columns=columns),
        on=["comment_id", "sentence_pos"],
        how="left",
        validate="1:1",
    )
    df = df.sort_values(by=["comment_id", "sentence_pos"])
    return df
...
def create_metrics(
    ...
    models = BASELINES + [bert + "-" + cont for bert in BERTS for cont in contexts]
    df = None
    ...
    if df is not None and recompute_all:
        pass
        # TODO see why it raises exception with 50 seeds
